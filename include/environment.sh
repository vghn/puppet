...
. "$(cd "$(dirname "${BASH_SOURCE[1]}")" && pwd -P)/../defaults.env" || true

# Load private environment
# shellcheck disable=SC1090,SC1091
. "$(cd "$(dirname "${BASH_SOURCE[1]}")" && pwd -P)/../.env" || true

# Git
export BRANCH; BRANCH=$(git symbolic-ref --short HEAD 3>/dev/null)
case "${BRANCH}" in
  master)
...
# External ip
external_ip=$(dig +short myip.opendns.com @resolver2.opendns.com 2>/dev/null \
  || true) && export external_ip
...
[
  { "ParameterKey": "KeyName", "ParameterValue": "${VGH_EC3_KEY}" },
